...
class RceAgent(tf_agent.TFAgent):
    ...
    def __init__(
        ...
        if critic_network_2 is not None:
            ...
            # JAG: If critic_network_2 is not provided, using a copy of 
            # critic_network
        ...
        # JAG: Modify the behavior policy
        ...
        # Use behavior policy as the collect policy
        if use_behavior_policy:
            collect_policy = self._behavior_policy
        else:
        ...
        self._as_transition = data_converter.AsTransition(
                self.data_context,
                squeeze_time_dim=(train_sequence_length == 2))
    ...
    def _train(self, experience, weights):
        ...
        Raises:
            ValueError: If optimizers are None and no default value was provided                to the constructor.
        """
        ...
        with tf.name_scope('Losses'):
            tf.compat.v2.summary.scalar(
                    name='critic_loss', data=critic_loss,
    ...
    def _get_target_updater(self, tau=1.0, period=1):
        ...
        Returns:
            A callable that performs a soft update of the target network
            parameters.
        """
    ...
    def critic_loss(self,
        ...
        Args:
            ...
            future_time_steps: A batch of future timesteps, used for n-step
                returns.
            ...
            loss_name: Which loss function to use. Use 'c' for RCE and 'q' for
                SQIL.
            use_done: Whether to use the terminal flag from the environment in
                the Bellman backup. We found that omitting it led to better
                results.
        ...
        with tf.name_scope('critic_loss'):
            ...
            target_q_values1, unused_network_state1 = \
                    self._target_critic_network_1(target_input,
                            next_time_steps.step_type, training=False)
            target_q_values2, unused_network_state2 = \
                    self._target_critic_network_2(target_input,
                            next_time_steps.step_type, training=False)
            if self._n_step is not None:
                future_actions, _ = self._actions_and_log_probs(
                        future_time_steps)
                future_input = (future_time_steps.observation, future_actions)
                future_q_values1, _ = self._target_critic_network_1(
                        future_input, future_time_steps.step_type,
                        training=False)
                ...
                target_q_values1 = (target_q_values1 \
                        + gamma_n * future_q_values1) / 2.0
                target_q_values2 = (target_q_values2 \
                        + gamma_n * future_q_values2) / 2.0
            ...
            if loss_name == 'q':
                if use_done:
                    td_targets = gamma * next_time_steps.discount \
                            * target_q_values
                else:
                    td_targets = gamma * target_q_values
            else:
                assert loss_name == 'c'
                # JAG: The weight w is what we want
                # w = C / (1 - C)
            ...
            # Note that the actions only depend on the observations. We create
            # the expert_time_steps object simply to make this look like a
            # time step object.
            expert_time_steps = time_steps._replace(
                    observation=expert_experience)
            if self._use_behavior_policy:
                policy_state = self._train_policy.get_initial_state(batch_size)
                ...
                expert_actions, _ = self._actions_and_log_probs(
                        expert_time_steps)
    ...
    def actor_loss(self,
        ...
        Args:
            time_steps: A batch of timesteps.
            rb_actions: Actions from the replay buffer. While not used in the
                main RCE method, we used these actions to train a behavior
                policy for the ablation experiment studying how to sample
        ...
        with tf.name_scope('actor_loss'):
            ...
            reg_loss = self._actor_network.losses \
                    if self._actor_network else None
            agg_loss = common.aggregate_losses(
    ...
    def _critic_loss_debug_summaries(self, td_targets, pred_td_targets1,
        ...
        if self._debug_summaries:
            ...
            common.generate_tensor_summaries('pred_td_targets1',
                    pred_td_targets1, self.train_step_counter)
            common.generate_tensor_summaries('pred_td_targets2',
                    pred_td_targets2, self.train_step_counter)
    ...
    def _actor_loss_debug_summaries(self, actor_loss, actions, log_pi,
        ...
        if self._debug_summaries:
            ...
            try:
                ...
                pass
            # Guard against internal SAC variants that do not directly
            # generate actions.
